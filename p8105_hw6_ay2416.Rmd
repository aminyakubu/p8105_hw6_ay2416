...
```{r}
linear_fit = lm(bwt ~ gaweeks + babysex + momage + mrace  + wtgain + smoken, data = children_df)
...
```{r}
fit_null = lm(bwt ~ blength + gaweeks, data = children_df)

fit_alt = lm(bwt ~ bhead * blength *babysex, data = children_df)
```
...
cross_validation = cv_df %>% 
  mutate(fit_null = map(train, ~ lm(bwt ~ blength + gaweeks, data = .x)),
         fit_alt = map(train, ~ lm(bwt ~ bhead * blength *babysex, data = .x))) %>% 
  mutate(rmse_fit_null = map2_dbl(fit_null, test, ~ rmse(model = .x, data = .y)),
         rmse_fit_alt = map2_dbl(fit_alt, test, ~ rmse(model = .x, data = .y)))
